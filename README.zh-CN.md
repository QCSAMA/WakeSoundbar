...
WakeSoundbar 是一个适用于 Windows 11 的轻量工具：当回音壁或功放设置为
**从桌面删除显示器** 后，它会在登录时自动恢复 HDMI 音频。它会唤醒被移出的
HDMI 显示路径，让音频设备重新出现，同时不改变主显示器的 4K、HDR、
高刷新率或 VRR 设置，也不会留下常驻后台进程。

**适合**

- 游戏 PC 或 HTPC
- 通过 HDMI 连接的回音壁或功放
- 每次重启后 HDMI 音频设备消失的系统
